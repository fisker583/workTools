...
class BugWriter:
	...
	def write(self, csvData, wb, wsBug, wsImage, bugFiled, imageFiled, imagePath):
		...
		for i, rows in enumerate(csvData):
			bugStr = formatStr('图片',rows['图片'])
			imageS = getImage(bugStr, imagePath)
			scaleS, hightS = getScale(imageS)
			wsImage.set_row(i+1, hightS)
			wsImage.insert_image(I+1, 12, image, {
				'x_scale': scaleS, 'y_scale': scaleS})
			# rows['食谱'] = '【SH】'+ str(rows['Bug编号']) + '-' + rows['食谱']
			# rows['重现步骤'] = rows['食谱'] + '\n' + rows['重现步骤']
			...
			for j, key in enumerate(bugFiled[0].keys()):
				...
				if key == '图片' and bugStr != '':
					fileUrl = 'internal:' + '大图!B' + str(rowImage + 2)
					wsBug.write_url(row, col, fileUrl, wb.add_format(
						getStyle('fileUrl')), string='查看大图')
					images = getImage(bugStr, imagePath)
					for r, image in enumerate(images):
						# copyImage(rows['Bug编号'],image)
						rowImage += 1
						scale, hight = getScale(image)
						wsImage.set_row(rowImage, hight, wb.add_format(
							getStyle('imageTitle')))
						wsImage.insert_image(rowImage, 1, image, {
			
											 'x_scale': scale, 'y_scale': scale})
						wsImage.write(
							rowImage, 0, rows['食谱'], wb.add_format(getStyle('imageTitle')))
						bugUrl = 'internal:' + '详情!F' + str(i+2)
						wsImage.write_url(rowImage, 1, bugUrl, wb.add_format(
							getStyle('imageUrl')), string='查看详情')
...
class BugXLsx:
	...
	def gen(self, xlsxSheets, sheetsThead, dataList, imagePath):
		...
		bug = BugWriter()
		bug.write(dataList, wb, ws['bug'], ws['image'],
				  sheetsThead['bug'], sheetsThead['image'], imagePath)
		print('wb.close()...')
		wb.close()
...
xlsxSheets = {'bug': '详情', 'image': '大图', 'total': '统计'}
sheetsThead = {
	'bug': [{'i': 35,
			 'page': 35,
			 'i2': 35,
...
xlsxPath = 'E:/Fisker/Downloads/rs/'
xlsxName = 'Test Image.xlsx'

bugImagePath = 'E:/Fisker/Downloads/rs/image/'

csvPath = 'E:/Fisker/Downloads/rs/'
csvName = 'test end'
